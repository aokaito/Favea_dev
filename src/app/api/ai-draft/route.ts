...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const event of extractionResult.events) {
      try {
        // 1. Idolを取得または作成
        const idolName = extractionResult.idol_name || keyword || 'Unknown'
        let idolId: string

        const existingIdolResult = await querySupabase<IdRecord>(supabase, 'idols', 'select', {
          columns: 'id',
          filter: { column: 'name', value: idolName },
          single: true,
        })
    ...
    return NextResponse.json({
      success: true,
      data: {
        idol_name: extractionResult.idol_name || keyword,
        events: savedEvents,
        message: `${savedEvents.length}件のイベント情報を取得しました`,
      },
    })
  } catch (error) {
